...
function confirmForm() {
    $("#cooperationAddForm").validate({
      submitHandler : function() {  //验证通过后的执行方法
          //当前的form通过ajax方式提交（用到jQuery.Form文件）
      	var param = $("#cooperationAddForm").serialize();
          $.ajax({
              url : ctx+"/CooperationlistController/addConfirm",
              type : "post",
              dataType : "json",
              data: param,
              success:function(data){
                  if(data.isSuc){
                	  document.getElementById("cooperationAddForm").reset(); 
                      swal({title: '成功',
                		  text: data.errMsg,
                		  type: 'success',
